<template>
  <div class="el-time-spinner el-time-spinner-test" :class="{ 'has-seconds': showSeconds }">
    <template v-if="!arrowControl">
      ...
      <el-scrollbar
        ...
        ref="minutes">
        <li
          @click="handleClick('minutes', { value: enabled, disabled: false })"
          v-for="(enabled, key) in minutesList1"
          :key="key"
          class="el-time-spinner__item"
          :class="{ 'active': enabled === minutes, disabled: !enabled }">{{ ('0' + enabled).slice(-2) }}</li>
...
<script type="text/babel">
  ...
  export default {
    ...
    methods: {
      ...
      modifyDateField(type, value, isClick) {
        if (this.isDivide && type === 'minutes' && !isClick) {
          this.$emit('change', modifyTime(this.date, this.hours, value * 5, this.seconds));
          return;
        }
        switch (type) {
      ...
      adjustSpinner(type, value) {
        ...
        if (el) {
          if (type === 'minutes' && this.isDivide) {
            el.scrollTop = Math.max(0, value * this.typeItemHeight(type) / 5);
          } else {
